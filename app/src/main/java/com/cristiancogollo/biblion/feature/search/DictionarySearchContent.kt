...
fun DictionarySearchContent(
    ...
    LaunchedEffect(selectedCategory, results) {
        if (query.trim().length < 2) {
            // Sin query: mostrar ultimos resultados re-filtrados (no aplica porque results esta vacio)
            return@LaunchedEffect
        }
        val baseQuery = query
        if (baseQuery.isNotBlank()) {
            // Re-ejecuta la busqueda con la query actual
            isLoading = true
            runCatching {
    ...
    Column(
        ...
        LazyRow(
            ...
            items(letters) { letter ->
                AssistChip(
                    onClick = {
                        // Disparar busqueda con prefijo letra
...
@Suppress("unused")
private fun unusedFlowOf(@Suppress("UNUSED_PARAMETER") context: Context) = flowOf<Unit>(Unit)
